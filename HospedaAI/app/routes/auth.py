...
def register():
    ...
    if form.validate_on_submit():
        ...
        # Enviar correo de confirmación (o imprimir link en consola en dev)
        sent = send_confirmation_email(user.correo)
        flash("Cuenta creada. Revisa tu correo para confirmar tu cuenta (o mira la consola si estás en desarrollo).", "success")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user.correo_confirmado:
            flash("Debes confirmar tu correo antes de iniciar sesión. Revisa tu email.", "warning")
            # Opcional: reenviar correo de confirmación
            return render_template('auth/login.html', form=form)
        login_user(user)
